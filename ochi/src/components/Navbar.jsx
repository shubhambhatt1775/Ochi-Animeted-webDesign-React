...
function Navbar() {
  ...
  const navItems = ["Services", "Our Work", "About Us", "Insights", "Contacts"];

  return (
    <div className="fixed z-[999] w-full px-6 md:px-20 py-6 flex bg-white/10 backdrop-blur-md supports-[backdrop-filter]:backdrop-blur-[3px] justify-between items-center font-['Neue Montreal'] ">
      {/* Logo */}
      <div className="logo">
        <svg
          width="72"
      ...
      <div className="hidden md:flex gap-10 items-center">
        {navItems.map((item, index) => (
          <a
            key={index}
            href="#"
            className={`text-sm font-light hover:text-gray-400 transition ${
              index === 4 ? "ml-32" : ""
            }`}
          >
            {item}
  ...
  bg-white/30 backdrop-blur-sm supports-[backdrop-filter]:backdrop-blur-[5px]">
          ...
          {navItems.map((item, index) => (
            <a
              key={index}
              href="#"
              className="text-lg font-light hover:text-gray-400 transition  "
